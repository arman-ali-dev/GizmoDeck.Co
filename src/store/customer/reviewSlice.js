  ...
  async ({ productId, rating, comment, images }, { rejectWithValue }) => {
    try {
      ...
      const { data } = await axios.post(
        `https://gizmodeck-co-server2.onrender.com/api/product/reviews/add/${productId}`,
  ...
  async (productId, { rejectWithValue }) => {
    try {
      ...
      const { data } = await axios.get(
        `https://gizmodeck-co-server2.onrender.com/api/product/reviews/all/${productId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
...
export const deleteReview = createAsyncThunk(
  ...
  async (reviewId, { rejectWithValue }) => {
    try {
      ...
      await axios.delete(
        `https://gizmodeck-co-server2.onrender.com/api/product/reviews/delete/${reviewId}`,
        {
