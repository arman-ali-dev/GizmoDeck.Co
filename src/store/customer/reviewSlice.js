  ...
  async ({ productId, rating, comment, images }, { rejectWithValue }) => {
    try {
      ...
      const { data } = await axios.post(
        `http://localhost:8081/api/product/reviews/add/${productId}`,
  ...
  async (productId, { rejectWithValue }) => {
    try {
      ...
      const { data } = await axios.get(
        `http://localhost:8081/api/product/reviews/all/${productId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
...
export const deleteReview = createAsyncThunk(
  ...
  async (reviewId, { rejectWithValue }) => {
    try {
      ...
      await axios.delete(
        `http://localhost:8081/api/product/reviews/delete/${reviewId}`,
        {
