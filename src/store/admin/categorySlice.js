  ...
  async (_, { rejectWithValue }) => {
    try {
      const { data } = await axios.get(
        "https://gizmodeck-co-server2.onrender.com/api/categories/all"
      );
  ...
  async (id, { rejectWithValue }) => {
    try {
      ...
      const res = await axios.delete(
        `https://gizmodeck-co-server2.onrender.com/api/admin/categories/${id}`,
        {
          headers: {
...
export const fetchSubcategories = createAsyncThunk(
  ...
  async (categoryId, { rejectWithValue }) => {
    try {
      ...
      const { data } = await axios.get(
        `https://gizmodeck-co-server2.onrender.com/api/categories/${categoryId}/sub`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
