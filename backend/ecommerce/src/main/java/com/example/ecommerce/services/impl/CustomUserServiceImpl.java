...
public class CustomUserServiceImpl implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        if (username.startsWith(SELLER_PREFIX)) {
            String actualSellerEmail = username.substring(SELLER_PREFIX.length());
            Seller seller = sellerRepository.findByEmail(actualSellerEmail);
            if (seller != null) {
                return buildUserDetails(seller.getEmail(), seller.getRole().toString());
            }
        } else {
